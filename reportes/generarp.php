...
$pdf->SetTitle('Reporte por Puesto');
...
$datos = $model->getReportePuesto($_POST['puesto']);//buscamos los datos para el reporte
if (count($datos) != 0) {
  ...
  $pdf->Cell(60, 0, 'PUESTO', 1, TRUE, 'C', 0, '', 0);
  ...
  foreach ($datos as $key =>$value) {
        ...
        $pdf->Cell(60, 0, ucwords($value->direccion_personal), 1, false, 'C', 0, '', 0);
        $pdf->Cell(60, 0, ucwords($value->nombre_puesto), 1, true, 'C', 0, '', 0);
        //$pdf->Cell(60, 0, 'Q. '.$value->sueldo_puesto, 1, true, 'C', 0, '', 0);

  }

} else {
    $html = '<h1>No se encontro ninguna coincidencia con el puesto especificado: '.$_POST['puesto'].'</h1>';
...
if ($_POST['opcion'] == 'visualizar') {
  ...
  $nombre_archivo = utf8_decode("personal por puestos.pdf");
