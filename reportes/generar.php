...
$pdf->SetTitle('Reporte por fecha de nacimiento');
...
$datos = $model->getReporteNacimiento($_POST['fecha']);//buscamos los datos para el reporte
if (count($datos) != 0) {
  $pdf->Cell(30, 0, 'NIT', 1, false, 'C', 0, '', 0);
  $pdf->Cell(60, 0, 'NOMBRES', 1, false, 'C', 0, '', 0);
  $pdf->Cell(60, 0, 'APELLIDOS', 1, false, 'C', 0, '', 0);
  $pdf->Cell(60, 0, 'DIRECCION', 1, false, 'C', 0, '', 0);
  $pdf->Cell(60, 0, 'FECHA DE NACIMIENTO', 1, TRUE, 'C', 0, '', 0);
  $pdf->SetFont('freesans', '', 12, '', true);
  foreach ($datos as $key =>$value) {
    	$pdf->Cell(30, 0, $value->__GET('nit'), 1, false, 'C', 0, '', 0);
      $pdf->Cell(60, 0, ucwords($value->__GET('nombre')), 1, false, 'C', 0, '', 0);
      $pdf->Cell(60, 0, ucwords($value->__GET('apellidos')), 1, false, 'C', 0, '', 0);
      $pdf->Cell(60, 0, ucwords($value->__GET('direccion')), 1, false, 'C', 0, '', 0);
      $pdf->Cell(60, 0, $value->__GET('fechaNacimiento'), 1, TRUE, 'C', 0, '', 0);
  }

} else {
    $html = '<h1>No se encontro ninguna coincidencia con la fecha de nacimiento: '.$_POST['fecha'].'</h1>';
...
if ($_POST['opcion'] == 'visualizar') {
  ...
  $nombre_archivo = utf8_decode("fecha de Nacimientos.pdf");
  $pdf->Output($nombre_archivo,'I');
}else {

  //este metodo tiene varias opciones, consulte la documentación para más información.
  $nombre_archivo = utf8_decode("fecha de Nacimiento.pdf");
